...
func (c *InterfacerCommand) Run(args []string) error {
	...
	if err != nil {
		return fmt.Errorf("parse source (path=%v): %w", c.File, err)
	}
	st := firstStruct(f)
	if st == nil {
		return fmt.Errorf("not found struct in file (path=%v)", c.File)
	}
	...
	if err != nil {
		return fmt.Errorf("process imports (path=%v): %w", c.File, err)
	}
	...
	if _, err := w.Write(b); err != nil {
		return fmt.Errorf("write (path=%v): %w", genFilename, err)
	}
